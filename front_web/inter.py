...
LastEditTime: 2024-04-22 01:15:52
'''
# =============================================================================模型预测

from model_foresee import predict 
...
def prd_sar_sent():
   audio_to_text()  #生成out_context.wav,out_target.wav和'dialog_WeGet\\speech_output.csv'
   gain_txtFea('D:\\Project_Design\\dialog_weGet\\speech_output.csv')
   gain_audioFea("D:\\Project_Design\\out_context.wav","D:\\Project_Design\\out_target.wav")  #生成特征文件
...
def read_dialogue_csv(file_path):
    ...
    return df
# 读取对话CSV文件
dialogue_df = read_dialogue_csv("dialog_weGet\\speech_output.csv")
...
with st.sidebar.container():
    ...
    if st.button("   确    定   "):                                              #预测输出功能
        # 获取输出框的值并进行处理
        showable = True
        sent_Label,sar_Label = prd_sar_sent()   #预测标签
        dialogue_df = read_dialogue_csv("dialog_weGet\\speech_output.csv")
