...
def audioFeatureExtraction(datatye):
    ...
    input_dir = "D:\Project_Design\QFNN-main\QFNN-main\data_collect\\audio" + '\\' + datatye
    i = 1 #任务序号
    ...
    for this_audio in uttNameList:
        ...
        with torch.no_grad():
            ...
            try:    #2_131出错
                ...
                fail_audioList.append(this_audio)          
                continue
            print("D:\Project_Design\QFNN-main\QFNN-main\\token\mustard\\audio_final/"+datatye+"/context/"+this_audio)
            print("D:\Project_Design\QFNN-main\QFNN-main\\token\mustard\\audio_final/"+datatye+"/target/"+this_audio)
            if os.path.exists("D:\Project_Design\QFNN-main\QFNN-main\\token\mustard\\audio_final/"+datatye+"/context/"+this_audio + ".txt"):
                pass
            else:
                np.savetxt("D:\Project_Design\QFNN-main\QFNN-main\\token\mustard\\audio_final/"+datatye+"/context/"+this_audio + ".txt", cAudioFeature, fmt="%f", delimiter=",") #txt特征文件保存到文件夹下

            if os.path.exists("D:\Project_Design\QFNN-main\QFNN-main\\token\mustard\\audio_final/"+datatye+"/target/"+this_audio + ".txt"):
                pass
            else:
               np.savetxt("D:\Project_Design\QFNN-main\QFNN-main\\token\mustard\\audio_final/"+datatye+"/target/"+this_audio + ".txt", tAudioFeature, fmt="%f", delimiter=",") 
