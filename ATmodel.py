...
import logging
# import qModule
# txt:torch.Size([4, 24, 768])        torch.Size([4, 64, 768])
# img:torch.Size([4, 24, 56,56])      torch.Size([4, 24, 140])
# au:torch.Size([4,24,128])           (4, 128)
import torch
...
class HyperModel(Module):
    def __init__(self, batchsize):
        ...
        # # 初始化批量归一化层
        # self.img_batchnorm_layer = nn.BatchNorm2d(24)  # 24->6
        # 初始化batchsize
        ...
        self.audioBiGRU = GRU(
            ...
            bidirectional=True)
        # Att是Attention之意
        # 自注意力机制将用于编码文本、图像和音频的特征。它们可以帮助模型更好地理解输入的文本、图像和音频，从而更好地进行预测和分类。
        ...
        # inter级多模态特征融合        # intra inter modal component
        # 初始化多头注意力
        # self.interTtoI = MultiheadAttention(128, 4)
        # self.interItoT = MultiheadAttention(128, 4)
        self.interTtoA = MultiheadAttention(128, 4)
        self.interAtoT = MultiheadAttention(128, 4)
        # self.interAtoI = MultiheadAttention(128, 4)
        # self.interItoA = MultiheadAttention(128, 4)
        # Decoder模块
        # 初始化GRU
    ...
    def Qin(self, text, tWavFea, cWavFea):  # 论文中的量子启发 交互 网络 quantum-inspired interactive network
        ...
        if torch.isnan(text).sum().item() > 0 or torch.isnan(audio).sum().item() > 0:
            ...
            #     imageAttnOutput, imageAttnOutputWeights = self.imageEncodeAtt(query, key, value)
            #     # imageAttnOutput (24,b,32)
            #
    ...
    def forward(self, text, tWavFea, cWavFea):
        ...
        # print(image.shape) #torch.Size([489, 4, 24, 140])
        ...
        # print("audio",audio)
            ...
            # print("=============图像领域形状\n")
            # for iChunk in imageChunks:
            #     # iChunk = torch.squeeze(iChunk, dim=1)  #torch.Size([128,24, 140])
            #     # iChunk=self.img_maxpool_layer(iChunk)  #torch.Size(([128,1, 6, 35])
            #     # print(iChunk.shape)
            #     # iChunk=self.img_batchnorm_layer(iChunk)
            #     # print(iChunk.shape)   #torch.Size([489, 24, 14, 14])
            #
            #     iChunk = iChunk.view(iChunk.shape[0], iChunk.shape[1], -1)  # view相当于numpy中的reshape
            #     print(iChunk.shape)  # torch.Size([489, 24, 196])
            #     # ichunk (b,24,14*14)
            #     iChunk = torch.squeeze(iChunk, dim=1)
            #     iChunk, ihn = self.imageBiGRU(iChunk)  # 此处报错   存疑，结构与M2model.py有差异
            #     # print(iChunk.shape)  #torch.Size([489, 24, 128])
            #     # iChunk, ihn = self.imageBiGRU2(iChunk)
            #
            #     # iChunk=iChunk.reshape(iChunk.shape[0],3,-1)
            #     ihnList.append(ihn)
            ...
            #     # imageAttnOutput (24,b,32)
            #     imageAttnOutput, imageAttnOutputWeights = self.imageEncodeAtt(query, key, value)
            #     # print(imageAttnOutput.shape)   #torch.Size([24, 489, 128])
            #     # imageAttnOutput (24,b,32)
            #
            #     iChunkList.append(imageAttnOutput)
            #     iEncoderAttWeight.append(imageAttnOutputWeights)
    ...
    for _ in range(epochs):
        ...
        for batch in data_loader:
            with autograd.detect_anomaly():
                ...
                # print("batch" + str(train_step) + '\n')
                # assert not torch.any(torch.isnan(textInput)), print("sentAttFlatten: ", textInput)
                # assert not torch.any(torch.isnan(imageInput)), print("sentAttFlatten: ", imageInput)
                # assert not torch.any(torch.isnan(audioInput)), print("sentAttFlatten: ", audioInput)
                sar, sent = model(textInput, tWavPath, cWavPath)
                if sar is None and sent is None:
                    continue
                else:
                        ...
                        with torch.no_grad():
                            ...
                            for batch in val_loader:
                                ...
                                if sar is None and sent is None:
                                    ...
                                    loss_val = (loss1_val + loss2_val) / 2  # 3-》2
